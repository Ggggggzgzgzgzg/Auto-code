...
local function autoSelectBestBrainrot(petsList)
	if not _G.AutoSelectBest or isStealing or autoStealEnabled then return end
	...
	for _, petData in ipairs(petsList) do
		if petData.slot < lowestSlot then
			lowestSlot = petData.slot
			bestPet = petData
		end
	end
	
	if bestPet then
		selectedPrompt = bestPet.prompt
		selectedSlotNumber = bestPet.slot
	end
end
...
local function updatePetList()
	...
	autoSelectBestBrainrot(tempPets)
    ...
    for _, petData in ipairs(tempPets) do
        ...
        row.BackgroundColor3 = isSelected and Color3.fromRGB(20, 20, 20) or C_list.card
        row.BorderSizePixel = 0
        row.Parent = scrollListRef
        Instance.new("UICorner", row).CornerRadius = UDim.new(0, 12)

        local rStroke = Instance.new("UIStroke")
        rStroke.Color     = isSelected and Color3.fromRGB(100, 100, 100) or C_list.stroke
    ...
    if #tempPets == 0 then
        ...
        emptyCard.BackgroundColor3 = Color3.fromRGB(15, 15, 15)
        emptyCard.BorderSizePixel  = 0
        emptyCard.ZIndex           = 4
        emptyCard.Parent           = scrollListRef
        Instance.new("UICorner", emptyCard).CornerRadius = UDim.new(0, 14)
        local es = Instance.new("UIStroke"); es.Color = Color3.fromRGB(30, 30, 30); es.Parent = emptyCard

        local iconCircle = Instance.new("Frame")
        iconCircle.Size             = UDim2.new(0, 40, 0, 40)
        iconCircle.Position         = UDim2.new(0.5, -20, 0, 18)
        iconCircle.BackgroundColor3 = Color3.fromRGB(10, 10, 10)
        iconCircle.BorderSizePixel  = 0
        iconCircle.ZIndex           = 5
        iconCircle.Parent           = emptyCard
        Instance.new("UICorner", iconCircle).CornerRadius = UDim.new(0, 20)
        local is = Instance.new("UIStroke"); is.Color = Color3.fromRGB(30, 30, 30); is.Parent = iconCircle
...
local old = PlayerGui:FindFirstChild("ZEMERX SCRIPT")
if old then old:Destroy() end
local oldB = PlayerGui:FindFirstChild("ZEMERX BANNER")
...
local ZEMERX_SCRIPT_GUI = Instance.new("ScreenGui")
ZEMERX_SCRIPT_GUI.Name           = "ZEMERX SCRIPT"
ZEMERX_SCRIPT_GUI.SelectionGroup = false
ZEMERX_SCRIPT_GUI.ResetOnSpawn   = false
ZEMERX_SCRIPT_GUI.DisplayOrder   = 999
ZEMERX_SCRIPT_GUI.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
ZEMERX_SCRIPT_GUI.IgnoreGuiInset = false
ZEMERX_SCRIPT_GUI.Parent         = PlayerGui
...
BorderFrame.Parent           = ZEMERX_SCRIPT_GUI
...
TextLabel.ZIndex             = 5
TextLabel.Text               = 'ZEMERX <font color="rgb(255,255,255)">PVP</font>'
...
CloseBtn.MouseButton1Click:Connect(function()
    ...
    t1:Play(); t2:Play()
    t1.Completed:Connect(function() ZEMERX_SCRIPT_GUI:Destroy() end)
end)
...
local BLINK_HI  = Color3.fromRGB(255, 255, 255)     -- Blanc vif
local BLINK_LO  = Color3.fromRGB(220, 220, 220)     -- Blanc/gris clair
...
local KB_FILE = "zemerx_keybinds.json"

local function saveKeybinds()
    pcall(function()
        writefile(KB_FILE, game:GetService("HttpService"):JSONEncode({
...
local function loadKeybinds()
    local ok, data = pcall(function()
        return game:GetService("HttpService"):JSONDecode(readfile(KB_FILE))
    end)
    if ok and type(data) == "table" then
        for _, action in ipairs({"flash", "block", "reset"}) do
            if data[action] then
                local ok2, kc = pcall(function() return Enum.KeyCode[data[action]] end)
                if ok2 and kc then
                    keybinds[action]  = kc
...
task.defer(function()
    ...
    if lbl then lbl.Text = "FLASH TP [" .. keyNames.flash .. "]" end
...
if DEVICE ~= "pc" then
    ...
    local function makeKeybindRow(actionName, displayName)
        ...
        keyBtn.Position           = UDim2.new(1, -63, 0.5, -12)
        ...
        local ks = Instance.new("UIStroke"); ks.Color = C.accent; ks.Parent = keyBtn
...
rejoinRow.Size             = UDim2.new(1, 0, 0, 40)
rejoinRow.BackgroundColor3 = C.card
rejoinRow.BorderSizePixel  = 0
rejoinRow.ZIndex           = 4
rejoinRow.LayoutOrder      = sectionOrder
rejoinRow.Parent           = ScrollingFrame2
Instance.new("UICorner", rejoinRow)
local rs = Instance.new("UIStroke"); rs.Color = C.stroke; rs.Parent = rejoinRow

local rejoinLbl = Instance.new("TextLabel")
rejoinLbl.Size               = UDim2.new(1, -80, 1, 0)
rejoinLbl.Position           = UDim2.new(0, 12, 0, 0)
rejoinLbl.BackgroundTransparency = 1
rejoinLbl.ZIndex             = 5
rejoinLbl.Text               = "Rejoin Server"
rejoinLbl.TextColor3         = Color3.fromRGB(255, 255, 255)
rejoinLbl.TextSize           = 13
rejoinLbl.Font               = Enum.Font.GothamMedium
rejoinLbl.TextXAlignment     = Enum.TextXAlignment.Left
rejoinLbl.Parent             = rejoinRow

local rejoinBtn = Instance.new("TextButton")
rejoinBtn.Size               = UDim2.new(0, 55, 0, 24)
rejoinBtn.Position           = UDim2.new(1, -63, 0.5, -12)
rejoinBtn.BackgroundColor3   = C.iconBg
...
local rbs = Instance.new("UIStroke"); rbs.Color = C.accent; rbs.Parent = rejoinBtn

rejoinBtn.MouseButton1Click:Connect(function()
    rejoinBtn.Text       = "..."
    rejoinBtn.TextColor3 = C.accent
    pcall(function()
...
-- ==========================================
-- BRAINROT IMAGES SYSTEM
-- ==========================================
-- Système pour afficher les images brainrot PVP

local BrainrotSystem = {}
BrainrotSystem.images = {
	"rbxassetid://1293995461", -- Image placeholder 1
	"rbxassetid://1293995461", -- Image placeholder 2
	"rbxassetid://1293995461", -- Image placeholder 3
	"rbxassetid://1293995461", -- Image placeholder 4
	"rbxassetid://1293995461", -- Image placeholder 5
}
BrainrotSystem.currentIndex = 1
BrainrotSystem.isOpen = false
BrainrotSystem.animating = false

local function createBrainrotViewer()
	local screenSize = PlayerGui.AbsoluteSize
	
	-- Frame principal (backdrop)
	local backdropFrame = Instance.new("Frame")
	backdropFrame.Name = "BrainrotBackdrop"
	backdropFrame.Size = UDim2.new(1, 0, 1, 0)
	backdropFrame.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
	backdropFrame.BackgroundTransparency = 0.7
	backdropFrame.BorderSizePixel = 0
	backdropFrame.ZIndex = 98
	backdropFrame.Visible = false
	backdropFrame.Parent = PlayerGui
	
	-- Fenêtre principale
	local mainWindow = Instance.new("Frame")
	mainWindow.Name = "BrainrotWindow"
	mainWindow.Size = UDim2.new(0, 520, 0, 480)
	mainWindow.Position = UDim2.new(0.5, -260, 0.5, -240)
	mainWindow.BackgroundColor3 = C.header
	mainWindow.BorderSizePixel = 0
	mainWindow.ZIndex = 99
	mainWindow.Parent = backdropFrame
	...
	headerLabel.Text = "BRAINROT GALLERY"
	...
	imageFrame.Size = UDim2.new(1, -30, 0, 360)
	imageFrame.Position = UDim2.new(0, 15, 0, 60)
	imageFrame.BackgroundColor3 = Color3.fromRGB(20, 20, 20)
	imageFrame.BorderSizePixel = 0
	imageFrame.ZIndex = 100
	imageFrame.Parent = mainWindow
	Instance.new("UICorner", imageFrame).CornerRadius = UDim.new(0, 8)
	local imgStroke = Instance.new("UIStroke")
	imgStroke.Color = C.stroke
	...
	counterLabel.Parent = mainWindow
	
	-- Navigation buttons
	local prevBtn = Instance.new("TextButton")
	prevBtn.Size = UDim2.new(0, 50, 0, 40)
	prevBtn.Position = UDim2.new(0, 15, 1, -55)
	prevBtn.BackgroundColor3 = C.card
	prevBtn.BorderSizePixel = 0
	prevBtn.ZIndex = 101
	prevBtn.Text = "◀"
	prevBtn.TextColor3 = C.accent
	prevBtn.TextSize = 18
	prevBtn.Font = Enum.Font.GothamBold
	prevBtn.Parent = mainWindow
	Instance.new("UICorner", prevBtn).CornerRadius = UDim.new(0, 6)
	local prevStroke = Instance.new("UIStroke"); prevStroke.Color = C.stroke; prevStroke.Parent = prevBtn
	
	local nextBtn = Instance.new("TextButton")
	nextBtn.Size = UDim2.new(0, 50, 0, 40)
	nextBtn.Position = UDim2.new(1, -65, 1, -55)
	nextBtn.BackgroundColor3 = C.card
	nextBtn.BorderSizePixel = 0
	nextBtn.ZIndex = 101
	nextBtn.Text = "▶"
	nextBtn.TextColor3 = C.accent
	nextBtn.TextSize = 18
	nextBtn.Font = Enum.Font.GothamBold
	nextBtn.Parent = mainWindow
	Instance.new("UICorner", nextBtn).CornerRadius = UDim.new(0, 6)
	local nextStroke = Instance.new("UIStroke"); nextStroke.Color = C.stroke; nextStroke.Parent = nextBtn
	
	-- Random button
	local randomBtn = Instance.new("TextButton")
	randomBtn.Size = UDim2.new(1, -130, 0, 40)
	randomBtn.Position = UDim2.new(0, 70, 1, -55)
	randomBtn.BackgroundColor3 = C.card
	randomBtn.BorderSizePixel = 0
	randomBtn.ZIndex = 101
	randomBtn.Text = "RANDOM"
	randomBtn.TextColor3 = Color3.fromRGB(200, 150, 80)
	randomBtn.TextSize = 12
	randomBtn.Font = Enum.Font.GothamBold
	randomBtn.Parent = mainWindow
	Instance.new("UICorner", randomBtn).CornerRadius = UDim.new(0, 6)
	local randomStroke = Instance.new("UIStroke"); randomStroke.Color = Color3.fromRGB(200, 150, 80); randomStroke.Parent = randomBtn
	
	-- Update display function
	local function updateImageDisplay()
		if BrainrotSystem.currentIndex > #BrainrotSystem.images then
			BrainrotSystem.currentIndex = 1
		elseif BrainrotSystem.currentIndex < 1 then
			BrainrotSystem.currentIndex = #BrainrotSystem.images
		end
		
		imageLabel.Image = BrainrotSystem.images[BrainrotSystem.currentIndex] or ""
		counterLabel.Text = BrainrotSystem.currentIndex .. " / " .. #BrainrotSystem.images
		
		-- Animation
		imageLabel.ImageTransparency = 0.3
		local tw = TweenService:Create(imageLabel, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {ImageTransparency = 0})
		tw:Play()
	end
	
	-- Button connections
	prevBtn.MouseButton1Click:Connect(function()
		if BrainrotSystem.animating then return end
		BrainrotSystem.animating = true
		BrainrotSystem.currentIndex = BrainrotSystem.currentIndex - 1
		updateImageDisplay()
		task.wait(0.2)
		BrainrotSystem.animating = false
	end)
	...
	randomBtn.MouseButton1Click:Connect(function()
		if BrainrotSystem.animating then return end
		BrainrotSystem.animating = true
		BrainrotSystem.currentIndex = math.random(1, #BrainrotSystem.images)
		updateImageDisplay()
		task.wait(0.2)
		BrainrotSystem.animating = false
	end)
	...
	closeBtn.MouseButton1Click:Connect(function()
		BrainrotSystem:Close()
	end)
	
	-- Hover effects
	for _, btn in ipairs({prevBtn, nextBtn, randomBtn}) do
		btn.MouseEnter:Connect(function()
			TweenService:Create(btn, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundColor3 = Color3.fromRGB(50, 50, 50)}):Play()
		end)
		btn.MouseLeave:Connect(function()
			TweenService:Create(btn, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundColor3 = C.card}):Play()
		end)
	end
	
	BrainrotSystem.backdropFrame = backdropFrame
	BrainrotSystem.mainWindow = mainWindow
	BrainrotSystem.updateImageDisplay = updateImageDisplay
end

function BrainrotSystem:Open()
	if self.isOpen or self.animating then return end
	self.isOpen = true
	self.animating = true
	
	if not self.backdropFrame then createBrainrotViewer() end
	
	self.backdropFrame.Visible = true
	self.mainWindow.Visible = true
	
	-- Animation d'ouverture
	local scale = self.mainWindow:FindFirstChildOfClass("UIScale")
	if not scale then scale = Instance.new("UIScale"); scale.Parent = self.mainWindow end
	scale.Scale = 0
	
	local tw = TweenService:Create(self.mainWindow, TweenInfo.new(0.25, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Position = UDim2.new(0.5, -260, 0.5, -240)})
	tw:Play()
...
function BrainrotSystem:Close()
	if not self.isOpen or self.animating then return end
	self.animating = true
	
	self.isOpen = false
	
	local tw = TweenService:Create(self.mainWindow, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {Position = UDim2.new(0.5, -260, 1.5, 0)})
	tw:Play()
	tw.Completed:Connect(function()
		self.backdropFrame.Visible = false
		self.mainWindow.Visible = false
		self.animating = false
	end)
end

-- Keybind pour ouvrir la galerie (par défaut: G)
local brainrotKeybind = Enum.KeyCode.G
UserInputService.InputBegan:Connect(function(inp, gpe)
	if gpe then return end
	if inp.KeyCode == brainrotKeybind then
		if BrainrotSystem.isOpen then
			BrainrotSystem:Close()
		else
			BrainrotSystem:Open()
		end
...
table.insert(ActiveConnections, UserInputService.InputBegan:Connect(function(inp, gpe)
	if gpe then return end
	if inp.KeyCode == brainrotKeybind then
		if BrainrotSystem.isOpen then
			BrainrotSystem:Close()
